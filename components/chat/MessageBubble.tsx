...
function SeverityBadge({ severity }: { severity: Severity }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-500 ${cls}`}
    >
      <Icon size={11} />
      {label}
    </span>
...
export default function MessageBubble({ msg }: { msg: Message }) {
  ...
  return (
    ...
    >
      {!isUser && <Img src="/logo.png" alt="Logo" className="size-10" />}
      ...
      {isUser && (
        <div className="flex size-10 shrink-0 items-center justify-center rounded-full bg-surface-2 border border-border text-xs font-700 text-muted">
          ME
        </div>
      )}
    </motion.div>
